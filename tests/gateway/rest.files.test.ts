...
describe('GET /files/:id', () => {
  ...
  beforeEach(async () => {
    ...
    const cfg: Config = { apiKey: '', manifestPath: '', tmpDir, logDir: '/tmp', logLevel: 'info', cookieDir: tmpDir, outputDir: tmpDir, proxyUrl: null };
    const manifest: Manifest = { getSiteHelp: vi.fn(), listSites: vi.fn(), searchSites: vi.fn() } as unknown as Manifest;
    handler = createRestHandler({ cfg, manifest, tempStore, run: vi.fn() });
    writeHead = vi.fn();
    end = vi.fn();
    setHeader = vi.fn();
    chunks = [];
