...
import { runVideoDownload } from '../../src/gateway/video/video-handlers.js';
...
function makeCfg(apiKey = 'test-key'): Config {
  return {
    apiKey,
...
describe('REST /video/download auth + happy path', () => {
  ...
  it('rejects unauthenticated POST /video/download with 401', async () => {
    const req = makeReq({ method: 'POST', url: '/video/download', body: { urls: ['https://x/y'] } });
    const res = makeRes();
  ...
  it('returns 404 for the removed POST /video/search route', async () => {
    const req = makeReq({
      method: 'POST',
      url: '/video/search',
      body: { query: 'x' },
