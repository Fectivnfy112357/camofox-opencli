...
#
# /home/node/.camofox comes in as a host-side bind mount whose UID/GID
# rarely matches the `node` user inside the container. Without chown here,
# the gateway can't write its JSONL log and the browser can't persist
# cookies. Run once on every start, costs nothing if ownership already
# matches.
#
