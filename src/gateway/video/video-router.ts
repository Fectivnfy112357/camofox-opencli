...
// <query>. All current adapters expose a `search` command that takes the
// query directly.
...
function mapRow(site: VideoSite, row: any): VideoSearchResult | null {
  ...
  // url: trust the adapter's own url when it actually points at a video post.
  // When the adapter leaves url empty, fall back to canonicalUrl(id) so the
  // result row can still be linked.
  ...
  return {
    ...
    // we keep that adapter's choice rather than coercing to a number.
